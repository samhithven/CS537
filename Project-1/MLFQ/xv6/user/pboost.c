...
main(int argc, char *argv[])
{
  ...
  for (j = 0; j < NPROC; j++) {
    if (st.inuse[j] && st.pid[j] == getpid()) {
	  ...
	  DEBUG_PRINT((1, "\t The priority of the parent process is %d and total wait ticks in q-0 : %d and q-1 : %d and q-2 : %d amd q-3 : %d\n",                                                                                      st.priority[j], st.wait_ticks[j][0], st.wait_ticks[j][1], st.wait_ticks[j][2], st.wait_ticks[j][3]));
          }
         
          check(getpinfo(&st) == 0, "getpinfo");
	  DEBUG_PRINT((1, "\t The priority of the parent process is %d \n",st.priority[j]));

    /*and total wait ticks in q-0 : %d and q-1 : %d and q-2 : %d amd q-3 : %d\n",                                                                                      st.priority[j], st.wait_ticks[j][0], st.wait_ticks[j][1], st.wait_ticks[j][2], st.wait_ticks[j][3]));  */
